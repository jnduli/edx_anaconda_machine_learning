...
#
# .. your code here ..
user1 = user1[pd.DatetimeIndex(user1.CallDate).weekday >= 5]
#print user1.count()
#
...
user1 = user1[ (user1.CallTime> pd.to_timedelta('22:00:00')) | (user1.CallTime< pd.to_timedelta('06:00:00'))]
...
ax.scatter(user1.TowerLon,user1.TowerLat, c='g', marker='x', alpha=0.2)
...
def doKMeans(df):
  print 'Starting kkmeans'
  print df.TowerLat.unique()
  fig = plt.figure()
  ax = fig.add_subplot(111)
  ax.scatter(df.TowerLat, df.TowerLon, marker='.', alpha=0.3)
  df = df.loc[:,['TowerLat','TowerLon']]
